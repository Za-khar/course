...
router.get('/auth', async function (req, res) {
  try {
    ...
    const token = jwt.sign({ id: user.user_id }, config.get('SECRET_KEY'), {
      expiresIn: '1h',
    })
...
router.get('/confirmation/:token', async (req, res) => {
  try {
    ...
    return res.redirect(
      `http://${config.get('HOST')}:${config.get('PORT')}/auth/login`
    )
  } catch (e) {
    console.log(e)
...
router.post(
  '/social/google',
  [checkSocialAccount(googleUrl)],
  authController.socialLogin
)

router.post(
  '/social/facebook',
  [checkSocialAccount(facebookUrl)],
  authController.socialLogin
)
